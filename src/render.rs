use crate::{tree::Tree, virtual_tree::VirtualTree};
    ...
    pub fn run(self, app: Component) {
        ...
        thread::spawn(move || {
            ...
            LocalSet::new().block_on(&rt, async move {
                let mut vtree = VirtualTree::new(app, tree_clone);
                loop {
                    vtree.run().await.unwrap();
                }
            })
        });
        ...
        el.run(move |event, _, control_flow| {
            ...
            if draw_frame {
                ...
                // PAINT
                tree.lock().unwrap().render(canvas);
